...
export default function ShotPanel() {
  ...
  return (
    <div id="shotPanel">
      ...
      {/* ペナルティは折りたたみ（既存ロジックの DOM id を維持） */}
      <div className={`sp-penalty-fold${penaltyOpen ? ' open' : ''}`}>
        <button
          type="button"
          className="sp-penalty-toggle"
          onClick={() => setPenaltyOpen(o => !o)}
          aria-expanded={penaltyOpen}
        >
          <span>⚠️ ペナルティ・OB</span>
          <span className="sp-penalty-chevron">{penaltyOpen ? '▲' : '▼'}</span>
        </button>

        <div id="spBodyPenalty" style={{ display: penaltyOpen ? 'block' : 'none' }}>
