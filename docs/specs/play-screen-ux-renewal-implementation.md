...
| 順 | ラベル | 見た目 | 動作 |
|----|--------|--------|------|
| 1 | 現在地で記録 | 緑 `#1F9E52` / リング `#6FE0A0` ＋白ピン | GPS → ショットパネル |
| 2 | パット数記録 | 金 `#E0A51F` / リング `#F5CF6A` ＋黒パター | カップパネル |
| 3 | ヤード測定 | スレート `#414C58` / リング `#6A7784` ＋定規 | T→C 測距表示 |
...
| 順 | ラベル | 見た目 | 動作 |
|----|--------|--------|------|
| 4 | 地図回転 | ティール `#274A52` / リング `#5AA6B0` ＋ 🧭 | `rotateToHole()` |

アイコン実体：`shotty/public/icons/icon-{pin,putter,ruler}.png`（60px @2x、占有率約72%に前処理済み）  
表示条件：マップが実際に表示されているときのみ（ティー種別選択中は非表示）。詳細は [`2026-08-24_launcher-panel-and-rail.md`](./2026-08-24_launcher-panel-and-rail.md)。

### 2-3. 機能ランチャー（ラウンド中・低頻度）

定義：`shotty/src/lib/launcher-items.ts` の `LAUNCHER_GROUPS`（仮絵文字。後から `iconSrc` で差し替え可）

| グループ | タイル | 動作 |
|----------|--------|------|
| 記録を見る | スコアカード | `openScorecard()` |
| 記録を見る | ショット履歴 | `toggleReview()` |
| 記録を見る | ラウンド履歴 | `openHistory()` |
| 調整 | クラブセット | `openClubEditor()`（メニューからは撤去・ランチャーのみ） |
| 調整 | GPS追尾 | `onGpsBtn()` |
| （仕切り・危険） | ホール消去 | 確認後 `clearHoleShots()`（赤系タイル） |
